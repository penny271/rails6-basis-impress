...
class Customer::AccountForm
  ...
  def assign_attributes(params = {})
    @params = params
    # ¥ 2.ch.9.3.4 フォームオブジェクトの修正
    # - .in? railsフレームワーク で使用可能 .include? と順序が逆になる以外ほとんど同じ動作
    # * チェックボックスがOnであるときフォームから送られてくるのは "1" という文字列ですが、隠し入力欄の値が true であるときフォームから送られてくるのは "true" という文字列となるため、このような変更が必要となります
