...
$(document).on("turbolinks:load", () => {
  $("div.button-wrapper").on("click", "button#update-entries-button", () => {
    ...
    $("table.entries input.approved").each((index, elem) => {
      if ($(elem).prop("checked")) approved.push($(elem).data("entry-id"));
      else not_approved.push($(elem).data("entry-id"));
    });
    ...
    $("table.entries input.canceled").each((index, elem) => {
      if ($(elem).prop("checked")) canceled.push($(elem).data("entry-id"));
      else not_canceled.push($(elem).data("entry-id"));
    });
